# CatMapPy

CatMapPy is a Python interface to the CatMapper API for searching catalog data, preparing merge keys, and submitting translation workflows.

[Get started](quickstart.md)  
[ProjectCatMapper organization site](https://projectcatmapper.github.io/)

## Why CatMapPy?

- Python-native wrapper around CatMapper endpoints
- DataFrame-friendly helpers for merge and translation workflows
- Consistent input validation and API error messages

## Documentation sections

- [Installation](installation.md)
- [Quickstart](quickstart.md)
- [Usage](usage.md)
- [API Reference](api-reference.md)
- [Contributing](contributing.md)
- [Citation](citation.md)
- [License](license.md)
